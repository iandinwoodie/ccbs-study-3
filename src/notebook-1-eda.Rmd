...
-- Dog gender spilt almost even (579 F, 573 M)
-- Many, many more female owners (1084) than male (84) responded to the survey 
...
-- of the respondents who provided an answer (773), more than half of dogs had attended adolescent or adult training (506/773 = 65.5%). Again the majority (410/506 = 81.0%) received reward-based training only and attended group classes (382/410 = 93.2%)
...
(many people who did not report a behavior problem did not answer the training questions)

control group: 1 no behavior problem, 39 yes (2.5% have a behavior problem)
exp group: 14 no, 240 yes (5.5%)
no training: 6 no, 187 yes (3.1%)
only adult training: 1 no, 134 yes (0.7%)
...
Some interesting things about this figure:

** age distribution of dogs without behavior problems heavily skewed towards younger dogs (less than 24 months)
Might want to exclude dogs < 6 months, who have not had an opportunity for adolescent / adult training? There are 96 dogs <= 6 months old

** those that reported acquiring their dog pre-adolescence were far more likely to report a behavior problem:
(412/433 = 95.2% have behavior problem), as opposed to those who acquired dog later (337/719 = 46.9%)

** Males slightly more likely to have behavior problem?
(386/573=67.4% of males have behavior problem, as opposed to 363/579=62.7% of females)
...
187 dogs with no training do have behavior problems
17 dogs with some training have no behavior problems
413 dogs with some training do have behavior problems

(Most dogs in the study have at least one behavior problem)
(these numbers include one dog_adolescent_or_adult_training=NA response, need to remove it from df)
...
df_combo <- rbind(df_control,df_exp)
df_combo_no_pre_training <- rbind(df_adult_only,df_no_training)
...
#here we know that all dogs have received pre-adolscence training, so we don't need to check
...
#if the dog was acquired pre-adolescent, there appears to be a visible postive impact of training on a few behaviors: excessive barking, coprophagia, fear / anxiety, mounting, rolling in repulsive material, and aggressive

#gender does not seem to have much of an affect, except for males with aggression-- males who have not received adult training are far more likely to show aggression 
