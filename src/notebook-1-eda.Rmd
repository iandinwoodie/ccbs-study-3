...
p <- ggplot(df_sorted, aes(x=Behavior,y=Occurence)) + geom_bar(stat="identity",fill='steelblue') + scale_x_discrete(labels=my_labels) + scale_y_discrete(labels=NULL) + geom_text(aes(label = Occurence), hjust = 0, colour = "black")  + theme(axis.text=element_text(size=12),    axis.title=element_text(size=16,face="bold")) 

p.labs <- p + coord_flip() + labs(y = "percentage of total", ) 
p.labs
