...
#summary stats specific to included responses (valid response = True or False to dog_adolescent_or_adult_training, no NA's)
...
#number of dogs which report continued training
...
However almost all dogs had some kind of behavior problem, so it could depend on the particular behaviour or the severity of the problem (The survey did not ask about severity)
...
We want to isolate the effect of adolescent / adult training
 -- using all valid answers (they answered yes or no to the training question)
    df_valid_responses
...
ggplot(df_control_puppy_training_only, aes(x = behav_problem)) +
  geom_bar() + geom_text(aes(label=..count..),stat='count',vjust=-0.2)

# how many of dogs with behavior problems have had pre-adolescent training and adult training?
ggplot(df_exp_puppy_and_adult_training, aes(x = behav_problem)) +
  geom_bar() + geom_text(aes(label=..count..),stat='count',vjust=-0.2)
...
```
# small sample problem? almost all dogs have at least one behavior problem
...
df_valid_responses %>%
  ...
  theme(legend.position='bottom')
```
There are too few dogs without behavior issues to draw any conclusions 
...
ggplot(df_valid_responses, aes(num_behv_problems)) + geom_bar(fill='steelblue')
...
```{r}
summary(df_control_puppy_training_only)
```

```{r}
summary(df_exp_puppy_and_adult_training)
```
...
for (i in 1:length(vars_control)) {
  col <- vars_control[i]
  p <- df_control_puppy_training_only %>%
    select(col) %>%
    drop_na(col) %>%
    ggplot(aes_string(x = col)) +
    geom_bar(fill='steelblue')
...
for (i in 1:length(vars_exp)) {
  col <- vars_exp[i]
  p <- df_exp_puppy_and_adult_training %>%
    select(col) %>%
    drop_na(col) %>%
    ggplot(aes_string(x = col)) +
    geom_bar(fill='steelblue')
...
df_combo_preK_training <- rbind(df_control_puppy_training_only,df_exp_puppy_and_adult_training)
df_combo_no_preK_training <- rbind(df_adult_only,df_no_training)
df_combo_all <- rbind(df_control_puppy_training_only,df_exp_puppy_and_adult_training,df_adult_only,df_no_training)
...
1. Examine whole group
2. Isolate group that has received reward-based training only vs. no adult training
...
df_exp_reward <- df_valid_responses %>%
  filter(reward_only_adult_training == TRUE)
...
saveRDS(df_valid_responses, '../data/df_valid_responses.Rds')

#save datasets separated by pre-adolescent training status 
saveRDS(df_control_puppy_training_only, '../data/df_control_puppy_training_only.Rds')
saveRDS(df_exp_puppy_and_adult_training, '../data/df_exp_puppy_and_adult_training.Rds')
...
saveRDS(df_exp_reward, '../data/df_exp_reward.Rds')
saveRDS(df_control_no_adult_training,'../data/df_control_no_adult_training.Rds')
